...
export class BagelChatProvider implements vscode.TreeDataProvider<ChatItem> {
    ...
    constructor(
        ...
        private apiClient: BagelApiClient
    ) {}
    ...
    getChildren(element?: ChatItem): Thenable<ChatItem[]> {
        if (!element) {
            ...
            const recentItems = this.chatHistory.slice(-5).map((msg, index) => 
                new ChatItem(
    ...
    private updateWebviewChat(): void {
        if (this.webviewPanel) {
