...
exports.optimizeCodeCommand = void 0;
const vscode = __importStar(require("vscode"));
async function optimizeCodeCommand(apiClient) {
    try {
        ...
        await vscode.window.withProgress({
            ...
            if (response.success && response.data) {
                ...
                // Show diff
                await vscode.commands.executeCommand('vscode.diff', originalDoc.uri, optimizedDoc.uri, 'Original ↔ Optimized (BAGEL)');
                // Ask user if they want to replace the original code
                const action = await vscode.window.showInformationMessage(`🥯 Code optimization complete!\n\n${explanation}`, 'Replace Original', 'Keep Both', 'Cancel');
...
exports.optimizeCodeCommand = optimizeCodeCommand;
